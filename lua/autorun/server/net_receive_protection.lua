...
local function warn(ply, fmt, ...)
	...
	if ply.net_incoming_last_print_suppress[fmt] < RealTime() then
		ply.net_incoming_last_print_suppress[fmt] = RealTime() + 1
...
function net.Incoming(length, ply)
	do -- rate limit
		...
		if ply.net_incoming_rate_next_check < RealTime() then
			if ply.net_incoming_rate_count > 250 then
				warn(ply, "PLAYER is sending more than 250 net messages a second", ply)
				punish(ply, 2)
			end
