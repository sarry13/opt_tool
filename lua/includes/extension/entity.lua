...
local chlen = meta.GetTable
local owner_gay = meta.GetOwner
local owner = "Owner"
...
function meta:__index( key )
	...
	local tab = chlen(self)
	if ( tab ) then
		local val = tab[ key ]
		if ( val != nil ) then return val end
	end

	if ( key == owner ) then return owner_gay( self ) end
...
function meta:IsConstrained()
	...
	local c = chlen(self).Constraints
